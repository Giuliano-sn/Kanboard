...
class Tree(object):
    ...
    def add_filename(self, filename, view_id, is_file):
        ...
        if not is_file:
            ...
            self.kanboard_views[newname] = node
            return
    ...
    def draw_tree(self, result):
        ...
        for name in printed_parents:
            ...
            result += temp
        for name in self.kanboard_views:
            temp, stringnum = self.kanboard_views[name].print_children(self.kanboard_views, self.actions_map, length, stringnum)
            result += temp
        self.size = stringnum
        return result
