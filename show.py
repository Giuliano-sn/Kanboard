...
def set_proper_scheme(view):
    '''set color scheme'''
    settings = sublime.load_settings('kanboard.sublime-settings')
...
def set_view(view_id, window):
    ...
    if not view:
        ...
        view.set_syntax_file('Packages/Kanboard/kanboard' + SYNTAX_EXTENSION)
...
def show(window, path, view_id=None, other_window=False, single_pane=False, other_group=False, focus=False):
    ...
    """
    view_exist = first(window.views(), lambda v: v.settings().get("kanboard_type"))

    if other_group:
        prev_focus = window.active_view()
        # simulate 'toggle sidebar':
        if prev_focus and 'kanboard' in prev_focus.scope_name(0):
    ...
    view.set_name(name)
    view.settings().set('kanboard_type', True)
